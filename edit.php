    ...
    if (isset($_POST['edit_artikel'])) {
        ...
        if (draft($_POST)>0) {
            mysqli_query($Kon,"DELETE from artikel where id = '$nomer' ");
             ...
             echo "
                 <script>
                alert('data draft gagal ditambahkan di draft!');
                document.location.href = '". BASE_URL ."artikel?halaman=draft';
                </script>
             ";
        }
    }elseif (isset($_POST['edit_draft'])) {
        if (edit_draft($_POST)>0) {
             echo "
                 <script>
                alert('data draft berhasil diubah!');
                document.location.href = '". BASE_URL ."artikel?halaman=draft';
                </script>
             ";
        }else{
             echo "
                 <script>
                alert('data draft gagal di ubah!');
                document.location.href = '". BASE_URL ."artikel?halaman=draft';
                </script>
             ";
        }
        ...
        if (tambahkan($_POST)>0) {
            mysqli_query($Kon,"DELETE from draft where id = '$nomer' ");
             ...
             echo "
                 <script>
                alert('data gagal ditambahkan di artikel!');
                document.location.href = '". BASE_URL ."artikel?halaman=artikel';
                </script>
             ";
        }
...
<div class="container-fluid">
    ...
    <?php 
    $cek = lihat("SELECT *from $halaman where id = '$nomer '");
    foreach ($cek as $key) :
     ?>
    <form method="post" enctype="multipart/form-data">
        <input type="hidden" name="id" value="<?php echo $key['id'] ?>">
        <input type="hidden" name="gambarlama" value="<?= $key['gambar']?>">
        <div class="row mb-3">
            <?php if ($pangkat == 'admin'): 
                $status = $key['status'] ?>
               <div class="input-group mb-3">
                ...
                <div class="input-group mb-3">
                    <div class="input-group-prepend">
                        <div class="input-group-text">
                            <input type="radio" <?php if($status=="off")echo "checked"; ?> value="off" name="status">
                        </div>
                    </div>
        ...
        <div class="col-sm-12">
                ...
                <input type="text" name="penulis" value="<?php echo $key['penulis'] ?>" class="form-control" id="penulis" aria-describedby="penulis" readonly >
                <!-- <small id="emailHelp" class="form-text text-muted">We'll never share your email with anyone else.</small> -->
            </div>
            </div>
            <div class="col-sm-12">
                <div class="form-group">
                <label for="judul">Judul draft</label>
                <input type="text" name="judul" class="form-control" id="judul" aria-describedby="judul" value="<?php echo $key['judul'] ?>">
            </div>
            </div>
            ...
            <div class="col-sm-12">
                <div class="container">
                <label>isi</label>
                <textarea id="rich-editor" name="isi"> <?php echo $key['isi'] ?></textarea>
                </div>
            </div>
            ...
            <div class="input-group mb-3">
                ...
                <input type="file" class="custom-file-input" id="file" name="gambar" onchange="return fileValidation()" aria-describedby="inputGroupFileAddon01">
        ...
        if ($halaman == 'artikel') {
            echo "
                <button name='edit_artikel' class='tambah'  onclick='return confirm('apakah sudah anda back up di folder pribadi ??');'' >Edit</button>
                <button name='masukan_draft' class='tambah'  onclick='return confirm('apakah sudah anda back up di folder pribadi ??');'' >Masukan Draft</button>
            ";
        }elseif ($halaman == 'draft') {
            echo "
                <button name='masukan_artikel' class='tambah'  onclick='return confirm('apakah sudah anda back up di folder pribadi ??');'' >publish</button>
                <button name='edit_draft' class='tambah'  onclick='return confirm('apakah sudah anda back up di folder pribadi ??');'' >edit Draft</button>
            ";
        }
         ?>
    </form>
